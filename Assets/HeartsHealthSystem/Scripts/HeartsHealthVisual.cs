...
public class HeartsHealthVisual : MonoBehaviour {
    ...
    private void Start() {
        ...
        int divideNumber = maxHealth / 4;
        int minusNumber = maxHealth - divideNumber * 4;
        int totalHearts = divideNumber + (minusNumber > 0 ? 1 : 0);
        HeartsHealthSystem heartsHealthSystem = new HeartsHealthSystem(totalHearts);
        heartsHealthSystem.Damage(totalHearts*4 - currentHealth);
